  ...
  <nav class="navbar navbar-expand navbar-dark" style="background-color: green">
    <div class="navbar-left">
      <div class="navbar-brand">
        <router-link to="/home" class="text-white font-bold"
          >Agribank</router-link
        >
      </div>
      ...
      <div class="menu">
        <select
          v-if="user"
          v-model="selectedPasswordPage"
          @change="navigate"
          class="menu-select"
          style="margin-left: 20px"
        >
          <option value="" disabled selected>🔐 Tài khoản</option>
          <option value="/updatepassword">Cập nhật mật khẩu</option>
        </select>

        <!-- Select Menu Giao Dịch -->
        <select
          v-model="selectedTransactionPage"
          @change="navigate"
          class="menu-select"
        >
        ...
        >
          ...
          <option value="/createusers">Thêm nhiều users</option>
          <option value="/notifications">Quản lý Thông báo</option>
        </select>

        <!-- Select Menu Nghỉ Phép (Chỉ hiển thị nếu user là admin) -->
        <select
          v-if="isAdmin"
          v-model="selectedTransactionPage"
          @change="navigate"
          ...
          style="margin-left: 20px"
        >
          <option value="" disabled selected>📌 Menu nghỉ phép</option>
          <option value="/homeleaveofabsence">Home</option>
          <option value="/leaveofabsence">List</option>
        </select>
      </div>
    </div>
    ...
    <div class="nav-auth">
      ...
      <router-link v-if="!user" to="/" class="navbar-brand"
        >Đăng nhập</router-link
      >
      <router-link v-if="!user" to="/register" class="navbar-brand"
        >Đăng Ký</router-link
      >
...
export default {
  ...
  methods: {
    navigate() {
      ...
      if (this.selectedAdminPage) {
        this.$router.push(this.selectedAdminPage);
        this.selectedAdminPage = "";
      }
      if (this.selectedPasswordPage) {
        this.$router.push(this.selectedPasswordPage);
        this.selectedPasswordPage = "";
      }
    },
