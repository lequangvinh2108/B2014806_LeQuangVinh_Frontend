<template>
  <div class="full">
    ...
    <div class="row button-row21">
      ...
      <!-- Bọc dropdown và ô tìm kiếm trong div để căn phải -->
      <div class="filter">
        <!-- Dropdown lọc đơn vị -->
        <select
          v-model="selectedMadonvi"
          @change="filterUsers"
          class="filter-dropdown"
        >
          <option value="">Tất cả đơn vị</option>
          <option
            v-for="(value, key) in madonviList"
            :key="value.Madonvi"
            :value="value.Madonvi"
          >
            {{ key }}
    ...
    <table
      class="w-full border-collapse border border-gray-300 bg-custom"
    ...
    >
      ...
      <tbody>
        ...
        >
          ...
          <td class="border border-gray-300 p-2" style="text-align: left">
            {{ user.MaCN }}
          </td>
          <td class="border border-gray-300 p-2" style="text-align: left">
            {{ user.user }}
  ...
  data() {
    return {
      ...
      madonviList: {
        ...
        "PGD Bình An - Ba Hòn": { Madonvi: 29, MaCN: 7708 },
  ...
  methods: {
    async fetchUsers() {
      try {
        ...
        this.users = response.sort((a, b) => a.MaCN - b.MaCN);
    ...
    filterUsers() {
      this.filteredUsers = this.users.filter((user) => {
        const matchMadonvi =
          !this.selectedMadonvi || user.Madonvi == this.selectedMadonvi;
        const keyword = this.searchKeyword.toLowerCase();
        const matchKeyword =
          user.user.toLowerCase().includes(keyword) ||
          user.name.toLowerCase().includes(keyword);
        return matchMadonvi && matchKeyword;
      });
    },
    // filterUsers() {
    //   if (this.selectedMadonvi) {
    //     this.filteredUsers = this.users.filter(
    //       (user) => user.Madonvi == this.selectedMadonvi
    //     );
    //   } else {
    //     this.filteredUsers = this.users;
    //   }
